...
- (ZImageScrollView *)createImageViewWithFrame:(CGRect)rect Tag:(NSInteger)tag {
    ...
    if (self.enableLongPress) {
        zImageView.enableLongPressGesture = _enableLongPress;
        zImageView.zDelegate = self;
    }
    return zImageView;
}
...
- (void)didLongPressedZImageView:(ZImageScrollView *)zImageView {
    if ([self.delegate respondsToSelector:@selector(imageBroser:didLongPressImageAtIndex:)]) {
        [self.delegate imageBroser:self didLongPressImageAtIndex:zImageView.tag];
    }
}
@end
