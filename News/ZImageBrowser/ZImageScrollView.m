...
- (void)updateLongPressGesture {
    if (self.enableLongPressGesture && !self.longPressGesture) {
        _longPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPressGestureAction:)];
        [self addGestureRecognizer:_longPressGesture];
    }else if (!self.enableLongPressGesture && self.longPressGesture){
        [self removeGestureRecognizer:_longPressGesture];
    }
}
- (void)longPressGestureAction:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if ([self.zDelegate respondsToSelector:@selector(didLongPressedZImageView:)]) {
            [self.zDelegate didLongPressedZImageView:self];
        }
    }
}
